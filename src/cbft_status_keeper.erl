...
-define(REFRESH_INTERVAL,
        ns_config:get_timeout(cbft_status_keeper_refresh, 5000)).
-define(WORKER, cbft_status_keeper_worker).
-define(STALE_THRESHOLD,
        ns_config:read_key_fast(cbft_status_keeper_stale_threshold, 2)).
...
handle_cast({update, Status}, #state{source = local} = State) ->
    NumConnections = proplists:get_value(cbft_num_connections, Status, 0),
    NeedsRestart = proplists:get_value(cbft_needs_restart, Status, false),
...
restart_indexer(#state{restart_pending = false,
    ...
    work_queue:submit_work(
      ?WORKER,
      fun () ->
              ?log_info("Restarting the cbft indexer"),

              case ns_ports_setup:restart_port_by_name(indexer) of
                  {ok, _} ->
                      ?log_info("Restarted the cbft indexer successfully");
                  Error ->
                      ?log_error("Failed to restart the cbft indexer: ~p", [Error])
              end,
...
grab_status(#state{source = local}) ->
    case cbft_rest:get_json("api/nsstatus") of
        {ok, {[_|_] = Status}} ->
            process_status(Status);
        {ok, Other} ->
            ?log_error("Got invalid status from the cbft indexer:~n~p", [Other]),
            {error, bad_status};
        Error ->
            Error
    end;
    ...
    case Nodes of
        ...
        _ ->
            ...
            try remote_api:get_indexes(Node) of
                ...
                Error ->
                    ?log_error("Couldn't get cbft indexes from node ~p: ~p", [Node, Error]),
                    {error, failed}
            catch
                T:E ->
                    ?log_error("Got exception while getting cbft indexes from node ~p: ~p",
                               [Node, {T, E}]),
                    {error, failed}
            end
    end.
...
process_status(Status) ->
    case lists:keyfind(<<"code">>, 1, Status) of
        ...
        _ ->
            ?log_error("cbft Indexer returned unsuccessful status:~n~p", [Status]),
            {error, bad_status}
    end.
...
get_source() ->
    ...
    case ns_cluster_membership:should_run_service(Config, cbft, ns_node_disco:ns_server_node()) of
        true ->
            local;
        false ->
            CbftNodes = ns_cluster_membership:cbft_active_nodes(Config),
            {remote, CbftNodes, length(CbftNodes)}
    end.
...
notify_change(#state{indexes_version = Version} = State) ->
    gen_event:notify(cbft_events, {cbft_change, Version}),
    State.
