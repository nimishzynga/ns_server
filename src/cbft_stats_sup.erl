...
init([]) ->
    Children =
        [{cbft_stats_children_sup,
          {supervisor, start_link, [{local, cbft_stats_children_sup}, ?MODULE, child]},
          permanent, infinity, supervisor, []},
         {cbft_status_keeper_sup, {cbft_status_keeper_sup, start_link, []},
          permanent, infinity, supervisor, []},
         {cbft_stats_worker, {erlang, apply, [fun start_link_worker/0, []]},
...
handle_cfg_event(Event) ->
    case is_notable_event(Event) of
        false ->
            ok;
        true ->
            work_queue:submit_work(cbft_stats_worker, fun refresh_children/0)
    end.
...
compute_wanted_children(Config) ->
    case ns_cluster_membership:should_run_service(Config, cbft, node()) of
        false ->
            [];
        true ->
            StaticChildren = [cbft_stats_collector],
...
refresh_children() ->
    RunningChildren0 = [Id || {Id, _, _, _} <- supervisor:which_children(cbft_stats_children_sup)],
...
child_spec({Mod, Name}) when Mod =:= stats_archiver; Mod =:= stats_reader ->
    {{Mod, Name}, {Mod, start_link, ["@cbft-" ++ Name]},
     permanent, 1000, worker, []};
child_spec(Mod) ->
    {Mod, {Mod, start_link, []}, permanent, 1000, worker, []}.

start_child(Id) ->
    {ok, _Pid} = supervisor:start_child(cbft_stats_children_sup, child_spec(Id)).

stop_child(Id) ->
    ok = supervisor:terminate_child(cbft_stats_children_sup, Id),
    ok = supervisor:delete_child(cbft_stats_children_sup, Id).
